...
export function ProductDetailPage() {
      ...
      <DashboardPageLayout showPageHeader={false}>
        <div className="flex flex-col gap-4">
          ...
          <Alert variant="destructive">
            <AlertTitle>{t("modules.product.detail.notFoundTitle")}</AlertTitle>
            <AlertDescription className="flex flex-col gap-4 pt-2">
              <p>{t("modules.product.detail.notFound")}</p>
              <Button asChild variant="outline" className="w-fit">
                <Link to="/products">{t("modules.product.detail.backToList")}</Link>
              </Button>
            </AlertDescription>
          </Alert>
        </div>
  ...
  const onEdit = () => {
    toast.success(t("modules.product.detail.toastEdit"));
  };
                ...
                <div className="flex min-w-0 flex-col gap-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <Badge variant="secondary" className="font-normal">
                      {t("modules.product.detail.metaUnit", { unit: product.unit })}
                    </Badge>
                    <Badge variant="secondary" className="font-normal">
                      {t("modules.product.detail.metaBrands", { count: brandCount })}
                    </Badge>
                    <Badge variant="secondary" className="font-normal">
                      {t("modules.product.detail.metaSegments", { count: segmentCount })}
                    </Badge>
                  </div>
                </div>
              </div>
          ...
          <Card className="rounded-2xl shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
              <CardTitle className="flex items-center gap-2 text-base">
                ...
                {t("modules.product.detail.brandsTitle")}
              </CardTitle>
              <span className="text-sm text-muted-foreground">
                {t("modules.product.detail.brandsTotal", { count: brandCount })}
              </span>
            </CardHeader>
            <CardContent className="pt-0">
              {brands.length === 0 ? (
                <p className="text-sm text-muted-foreground">{t("modules.product.detail.brandsEmpty")}</p>
              ) : (
                    ...
                    <li key={b.id} className="flex items-center gap-3 py-3 first:pt-0">
                      ...
                      {b.isActive ? (
                        <Badge className="shrink-0 border-emerald-200 bg-emerald-50 font-medium text-emerald-900 hover:bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-100">
                          {t("modules.product.detail.brandStatus.active")}
                        </Badge>
                      ) : null}
                    </li>
                  ))}
          ...
          <Card className="rounded-2xl shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
              <CardTitle className="flex items-center gap-2 text-base">
                ...
                {t("modules.product.detail.segmentsTitle")}
              </CardTitle>
            ...
            <CardContent className="flex flex-col gap-6 pt-0">
              ...
              <div>
                <h3 className="mb-3 text-sm font-semibold text-foreground">
                  {t("modules.product.detail.summaryTitle")}
                </h3>
                <div className="flex flex-col gap-2 text-sm">
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-muted-foreground">{t("modules.product.detail.summaryBaseUnit")}</span>
                    <span className="font-semibold tabular-nums text-foreground">{product.unit}</span>
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <span className="text-muted-foreground">
                      {t("modules.product.detail.summaryInternalId")}
                    </span>
