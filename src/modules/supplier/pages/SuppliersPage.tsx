...
export function SuppliersPage() {
  ...
  const rangeLabel = useMemo(() => {
    ...
    if (!from || !to) return t("modules.supplier.dateRange.pick");
    ...
    <Card key={row.id} className="flex flex-col overflow-hidden border-border shadow-sm">
        ...
        <div className="flex flex-wrap gap-2">
          {row.isActive ? (
            <Badge variant="secondary" className="font-normal">
              {t("modules.supplier.badge.active")}
            </Badge>
          ) : (
            <Badge variant="outline" className="font-normal">
              {t("modules.supplier.badge.inactive")}
            </Badge>
          )}
          {row.isNew ? (
      ...
      <div className="space-y-2 bg-muted/60 px-6 py-3">
        <p className="text-xs font-semibold uppercase tracking-wide text-muted-foreground">
          {t("modules.supplier.card.sectionCommercial")}
        </p>
      ...
      <CardContent className="flex flex-1 flex-col gap-3 pt-4">
        <p className="text-xs font-semibold uppercase tracking-wide text-foreground">
          {t("modules.supplier.card.sectionPayment")}
        </p>
        <div className="flex flex-col gap-2 text-sm">
          <div className="flex items-center justify-between gap-2">
            <span className="flex items-center gap-2 text-muted-foreground">
              <Package className="size-4 shrink-0" aria-hidden />
              {t("modules.supplier.card.delivery")}
            </span>
          ...
          <div className="flex items-center justify-between gap-2">
            <span className="flex items-center gap-2 text-muted-foreground">
              ...
              {t("modules.supplier.card.paymentTerm")}
            </span>
          ...
          <div className="flex items-center justify-between gap-2">
            <span className="flex items-center gap-2 text-muted-foreground">
              ...
              {t("modules.supplier.card.minOrder")}
            </span>
      ...
      <CardFooter className="flex flex-col gap-2 border-t bg-background/80 pt-4">
        ...
        <Button type="button" variant="outline" className="w-full" onClick={() => navigate(`/suppliers/${row.id}`)}>
          {t("modules.supplier.actions.edit")}
        </Button>
      </CardFooter>
    </Card>
  );
  ...
  return (
    <DashboardPageLayout
      showPageHeader
      title={t("modules.supplier.pageTitle")}
      headerActions={
        <div className="flex flex-wrap items-center gap-2">
          ...
          >
            ...
            {t("modules.supplier.actions.register")}
          </Button>
        </div>
      }
    >
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="relative flex-1">
            ...
            <Input
              ...
              placeholder={t("modules.supplier.searchPlaceholder")}
              className="pl-9"
              aria-label={t("modules.supplier.searchPlaceholder")}
            />
          </div>
          <Button type="button" variant="ghost" size="sm" className="shrink-0 gap-1 text-muted-foreground" onClick={clearFilters}>
            <X className="size-4" aria-hidden />
            {t("modules.supplier.clearFilters")}
          </Button>
        </div>

        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
          <Select value={sort} onValueChange={(v) => setSort(v as SortKey)}>
            <SelectTrigger aria-label={t("modules.supplier.filterSort")}>
              <SelectValue placeholder={t("modules.supplier.sort.recent")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="recent">{t("modules.supplier.sort.recent")}</SelectItem>
              <SelectItem value="name">{t("modules.supplier.sort.name")}</SelectItem>
            </SelectContent>
          </Select>
          <Select value={status} onValueChange={(v) => setStatus(v as StatusFilter)}>
            <SelectTrigger aria-label={t("modules.supplier.filterStatus.label")}>
              <SelectValue placeholder={t("modules.supplier.filterStatus.all")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t("modules.supplier.filterStatus.all")}</SelectItem>
              <SelectItem value="active">{t("modules.supplier.filterStatus.active")}</SelectItem>
              <SelectItem value="inactive">{t("modules.supplier.filterStatus.inactive")}</SelectItem>
            </SelectContent>
          </Select>
          <Select value={supplierId} onValueChange={setSupplierId}>
            <SelectTrigger aria-label={t("modules.supplier.filterSupplier.label")}>
              <SelectValue placeholder={t("modules.supplier.filterSupplier.all")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t("modules.supplier.filterSupplier.all")}</SelectItem>
          ...
          <Select value={branch} onValueChange={(v) => setBranch(v as SupplierBranchId | "all")}>
            <SelectTrigger aria-label={t("modules.supplier.filterBranch.label")}>
              <SelectValue placeholder={t("modules.supplier.filterBranch.all")} />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="all">{t("modules.supplier.filterBranch.all")}</SelectItem>
              <SelectItem value="matriz">{t("modules.supplier.filterBranch.matriz")}</SelectItem>
              <SelectItem value="norte">{t("modules.supplier.filterBranch.norte")}</SelectItem>
              <SelectItem value="litoral">{t("modules.supplier.filterBranch.litoral")}</SelectItem>
            </SelectContent>
          </Select>
        </div>
